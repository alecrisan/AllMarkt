...
{
    public class GetShopOrdersQuery : IRequest<IEnumerable<OrderViewModel>> {
           public int ShopId { get; set; }
    }

    public class GetShopOrdersQueryHandler : IRequestHandler<GetShopOrdersQuery, IEnumerable<OrderViewModel>>
    {
        public readonly AllMarktQueryContext _allMarktQueryContext;

        public GetShopOrdersQueryHandler(AllMarktQueryContext allMarktQueryContext)
        {
            _allMarktQueryContext = allMarktQueryContext;
        }

        public async Task<IEnumerable<OrderViewModel>> Handle(GetShopOrdersQuery request, CancellationToken cancellationToken)
        {
            var orders = await _allMarktQueryContext
                .Orders
                .Where(o => o.Seller.Id == request.ShopId)
                .ToListAsync();
